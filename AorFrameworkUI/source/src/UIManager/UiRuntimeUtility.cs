...
namespace Framework.UI
{
    public class UIRuntimeUtility
    {
        ...
        /// 检查并且创建EventSystem节点
        /// </summary>
        public static void CheckAndCreateEventSystem(bool DontDestroyOnLoad = true) {
            if (EventSystem.current == null)
            {
                GameObject esGo = GameObject.Find("EventSystem");
                if (!esGo) esGo = new GameObject("EventSystem");

                EventSystem es = esGo.GetComponent<EventSystem>();
                if (!es) es = esGo.AddComponent<EventSystem>();

                StandaloneInputModule sim = esGo.GetComponent<StandaloneInputModule>();
                if (!sim) sim = esGo.AddComponent<StandaloneInputModule>();
