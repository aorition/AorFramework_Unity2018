...
namespace Framework.UI
{
    public class UIManager : ManagerBase
    {
        ...
        {
            return ManagerBase.CreateInstance<UIManager>(ref _instance, parenTransform);
        }
        ...
        /// UGUI强制刷新UI Layout
        /// </summary>
        public static void UpdateLayout(Transform rect)
        {
            UIManager.Request(() => {
                var r = (rect is RectTransform ? (RectTransform)rect : rect.GetComponent<RectTransform>());
                UIManager.Instance.StartCoroutine(UIManager.Instance.m_updateLayout(r));
            });
        }
        ...
        {
            base.Awake();
            ManagerBase.VerifyUniqueOnAwake<UIManager>(ref _instance, this);
        }
        ...
        protected override void init()
        {
            ...
            //检查是否包含EventSystem
            UIRuntimeUtility.CheckAndCreateEventSystem();
        }
...
//            }
        ...
        public RectTransform GetLayerInAllCanvas(string layerName)
        {
            foreach (string key in _CanvasDic.Keys)
            {
                if (_LayerDic[key].ContainsKey(layerName))
                {
                    return _LayerDic[key][layerName];
                }
            }
            return null;
        }
        ...
        public void ClearLayer(string layerName, string canvasName = "Default")
        {
            if (_CanvasDic.ContainsKey(canvasName))
            {
                if (_LayerDic[canvasName].ContainsKey(layerName))
                {
                    RectTransform rt = _LayerDic[canvasName][layerName];
                    int i;
                    int len = rt.childCount;
                    if(len > 0)
                    {
                        List<GameObject> dels = new List<GameObject>();
                        for (i = 0; i < len; i++)
                        {
                            Transform sub = rt.GetChild(i);
                            dels.Add(sub.gameObject);
                        }
                        len = dels.Count;
                        if(len > 0)
                        {
        ...
        private IEnumerator m_updateLayout(RectTransform rect)
        {
            while (true)
            {
