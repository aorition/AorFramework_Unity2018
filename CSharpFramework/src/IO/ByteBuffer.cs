...
namespace AorBaseUtility
{
     ...
     */
        ...
        public string readUTF()
        {
            ...
            int n = ByteUtility.readUTF(array, position, len, temp);
            if (n < 0)
                throw new Exception(this
     ...
     */
        ...
        public void writeUTF(string str, int index, int length)
        {
            ...
            int len = ByteUtility.getUTFLength(str, index, length);
            writeLength(len + 1);
            if (len <= 0)
                return;
            int pos = top;
            if (array.Length < pos + len)
                setCapacity(pos + len);
            ByteUtility.writeUTF(str, index, length, array, pos);
            top += len;
        }
